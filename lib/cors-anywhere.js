...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // if location.host matches a defined environment var, use its value
    var env = process.env;
    var envVar = location.host.toUpperCase();
    Object.keys(env).forEach(function(key) {
      var val = env[key];
      if (envVar === key) {
        // we found the key, so start over with its value
        location = parseURL(val);
      }
    });
